...
$book_id_to_other = $_GET['idCantiere'];

 $book_id = $book_id_to_other;

    // select frequency and word by looking up it's source_ID to the current source_id in this loop
    $sql = "SELECT * FROM occurrence WHERE source_id = $book_id ORDER BY freq DESC";
    $result = mysqli_query($conn,$sql);
    // searching the hashmap for the name that matches current source_id
    $book_name = array_search($book_id,$book_arr);
    $totalRowQuery = "SELECT SUM(freq) FROM occurrence WHERE source_id = $book_id_to_other";
